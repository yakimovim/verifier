...
Sometimes in automation tests we need to validate work of methods, returning very complex objects as a result. We need to be sure, that these objects has correct structure. Common assertion libraries allow us to check our expectation about simple objects (strings, numbers), and collections of objects of primitive types. But it can happen, that we need more.
...
You can write down these expectaion this way:
```csharp
...
Why do we need to know about all problems with our complex object? You may say, that event one problem is the problem you need to fix. Why do you need to know about all of them? Let me ask you one thing. Do you prefer to have a compiler, which reports you only about the first error in a code it has met, or you prefer to have a compiler, which reports you only about all errors in the code? This is the same situation. It is the same situation here. It is up to you, to decide what you want to have. And if you want to have all information at once, this library is at your service.

## Verification result

In general, this library works with functions taking object, you want to check, and returning an instance of VerificationResult structure. This instance contains collection of error messages. There error messages can be passed to the constructor of the structure. Be aware, that all null, empty or whitespace messages will be ignored.
...
To construct verification result one can use constructor, or static function Normal and Critical. You also can implicitely convert string into verification result:
```csharp
...
where TUnderTest is the class of objects you are testing. Be aware, that if any of these verification functions throws an error, critical verification result will be generated for it. Error messages of this verification result will contain message of this exception.

There are many assertion libraries on the market. But they are designed to throw exceptions, if verification fails. Verifier supports these libraries using AddNormalVerifiers and AddCriticalVerifiers methods. They accept Action<TUnderTest> as a parameter. They produce normal and critical verification results correspondingly. If action throws an exception, then its message will be in the list of error messages of theverification result:
```csharp
...
When you add verification functions using AddVerifiers and AddNNNVerifiers methods, you create static verifications. It means, that you can reuse added verification function several times:
```csharp
